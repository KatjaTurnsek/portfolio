...
export function animateTextInSection(section) {
  ...
  const textElements = section.querySelectorAll(
    `h1${SKIP}, h2${SKIP}, h3${SKIP}, h4${SKIP}, p${SKIP}`
  );

  if (prefersReducedMotion()) {
    textElements.forEach((el) => /** @type {HTMLElement} */ ((el).style.opacity = '1'));
    return;
  }

  // Mobile: one short CSS fade with no SplitType or GSAP text tweens.
  if (SIMPLE_MOBILE_TEXT) {
    textElements.forEach((node) => {
      const el = /** @type {HTMLElement} */ (node);
      if (!markOnce(el)) return;

      el.classList.add('mobile-text-fade-in');
      el.addEventListener('animationend', () => el.classList.remove('mobile-text-fade-in'), {
        once: true,
      });
    });
